...
const realPathAll = promisifyFileReadMap.bind(null, fs.realpath, false);
const statsAll/*: PromStringArrayStats*/ =
  /*:: ((*/promisifyFileReadMap.bind(null, fs.stat, true)/*:: : any) : PromStringArrayStats)*/;
...
function _getBaseDirs(dir/*: string*/, seenList/*: Array<string>*/)/*: Promise<Array<string>>*/ {
  ...
  return readDir(dir)
    .then(dirList => realPathAll(
      dirList
        .filter(d => d !== 'node_modules' && d[0] !== '.')
        .map(d => path.join(dir, d))
        .filter(d => !seenList.includes(d)))
    )
    .then(statsAll)
    .then(all => {
      const allFiltered = all.filter(d => !(d instanceof Error));
      return allFiltered.filter(([_, stats]) => stats.isDirectory()).map(([d]) => {
        seenList.push(d);
        return d;
      });
    });
...
function promisifyFileReadMap(fn/*: ?Function*/, includeInput/*: boolean*/,
  list/*: Array<string>*/)/*: Promise<Array<string>>*/ {
  const cProm = (a) => {
    if (a instanceof Error) {
      return a;
    }
    const promise = promisify(fn, includeInput)(a);
    return promise.catch(err => {
      console.error(err.message);
      return err;
    });
  };
  const ret = list.map(cProm);
  const filteredRet = [];
  for (const x of ret) {
  ...
  return Promise.all(filteredRet);
}
